...
Each word embedding retrieved from the lookup table is then fed into the input layer of the RNN, which is followed by the hidden layers and the output layer. Hermans and Schrauwen~\cite{deep_rnn:hermans2013} demonstrate that RNNs with more than one hidden layer naturally employ a temporal hierarchy with multiple layers operating at different time scales, where lower layers capture short term interactions among words and higher layers reflect longer-term dependencies. For this reason, I chose to have two hidden layers in my architecture. \\
...
\end{tikzpicture}
\caption{An FSM representing the language model interaction. The thick arrows denote the path taken when the user is inserting text without any deletions or unnecessary spaces.}
...
Some datasets are more predictable than others, which means that metrics like perplexity and average-keys-saved depend on which dataset is used. In order to produce results that are comparable with the work of other authors, I have tested my language models on the Penn Tree Bank dataset, as shown in Table~\ref{table:ptb}. The RNN-based models used 2 hidden layers each with 256 neurons. The perplexity was calculated over the whole of the test set, whereas average-keys-saved, a more expensive metric to compute, was taken over the first 1000 words of the test set. \\
...
\begin{itemize}
\item
	The RNN-based language models outperform the $n$-gram models in terms of prediction accuracy. In my implementation, they run slightly faster at inference time and also consume less memory, although this is partly due to the fact that the $n$-gram models are entirely unpruned. Obtaining the correct hyperparameters and training the RNN-based language models, however, does take significantly longer.
\item
	Mikolov benchmarked a 3-gram + MKN, 5-gram + MKN and a 1-layer vanilla RNN model on the PTB dataset, for which he obtained perplexity scores of 148.3, 141.2 and 124.7 respectively~\cite{rnn_ptb:mikolov2012}. In another paper, he benchmarks a 1-layer LSTM model with 100 hidden neurons on the PTB dataset, for which he obtained a perplexity score of 115~\cite{lstm_ptb:mikolov2014}. All of these results closely match my own. The RNN-based results are slightly less close due to his models having a different number of neurons and hidden layers.
\item

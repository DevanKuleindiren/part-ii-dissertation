...
\begin{subfigure}{0.5\linewidth}
	...
	\end{tikzpicture}
	\caption{A simplified notation of (a).}
\end{subfigure}
...
\node[above left=8mm and 15mm of neuron] (inp1) {$a_1$};
\node[above left=0mm and 15mm of neuron] (inp2) {$a_2$};
\node[below left=-3mm and 15mm of neuron] (inp3) {$\vdots$};
\node[below left=8mm and 15mm of neuron] (inpN) {$a_n$};

\draw[<-] (neuron.west |- mid) coordinate (aux)--++(165:15.5mm) node [below right=-1mm and 2mm of inp2] {$w_2$};
\draw[<-] ([yshift=1mm]aux)--++(146:18mm) node [below right=-2mm and 2mm of inp1] {$w_1$};
\draw[<-] ([yshift=-1mm]aux)--++(-146:18mm) node [above right=-3mm and 2mm of inpN] {$w_n$};

\draw[->] (neuron.east) -- ++(0:5mm) node [right] {$\sigma \big( \sum_i a_i w_i \big)$};
...
\begin{itemize}
\item
	All language models written from scratch.
\end{itemize}

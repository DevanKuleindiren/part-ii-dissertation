...
My project investigates the performance of various language models in the context of text prediction. Language models (LMs) produce a probability distribution over a sequence of words, which can be used to estimate the relative likelihood of words or phrases occurring in various contexts. Specifically, given a vocabulary $V$ and a sequence of words $w_1w_2...w_N = w_1^N \in V^N$, they estimate $\mathbb{P}(w_1^N)$. Using repeated application of the product rule, this is often reduced to the task of estimating the probability of seeing the word, $w_i$, given the words that precede it, $w_1^{i - 1}$:
...
\begin{alignat*}{2}
	\overbrace{\tit{do you want to grab a }}^{w_1^{i - 1}}&\overbrace{\underline{\tit{drink}}}^{w_i} &&\hspace{0.5mm}\overbrace{(0.327)}^{\mathbb{P}(w_i | w_1^{i - 1})} \\
	&\tit{coffee} &&\ \ (0.211) \\
	&\tit{bite} &&\ \ (0.190) \\
	&\tit{spot} &&\ \ (0.084) \\
	&\vdots &&\ \ \vdots
...
In order for an RNN to take a sequence of words as input, each word must first be mapped to a vector representation, known as a \tit{word embedding}, which represents a point in a high-dimensional space. Ideally, similar words will be assigned points which are closer together. In this dissertation, I get the RNN to learn good word embeddings through the application of backpropagation and gradient descent to the vector elements.
...
The PTB~\cite{ptb:marcus1993} is a widely-adopted dataset of 887,521 words for measuring the quality of language models, derived from text from the Wall Street Journal. I used it for all tests in which the size of the training data is fixed.
...
\item
	LSTM yields marginally better prediction accuracy than the GRU architecture, but requires more memory. Apart from the memory overhead, GRUs significantly outperform the vanilla RNN architecture.
\item

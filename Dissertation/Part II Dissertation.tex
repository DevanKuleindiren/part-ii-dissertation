...
Typing on a mobile device can be a tedious experience. Predictive keyboards serve to somewhat alleviate this, by enabling faster word completion and fewer mistakes. In this project, I investigate a variety of language models for text prediction, and implement a predictive mobile keyboard on iOS. I also introduce a novel extension to an existing language model that improves its performance on error-prone input. \\

Language models (LMs) produce a probability distribution over a sequence of words, which can be used to estimate the relative likelihood of words or phrases occurring in various contexts. Specifically, given a vocabulary $V$ and a sequence of words $w_1w_2...w_N \in V^N$, they estimate $\mathbb{P}(w_1w_2...w_N)$. Using repeated application of the product rule, this is often reduced to the task of estimating the probability of seeing the word, $w_i$, given the words that precede it:
\begin{gather} \label{eq:lm_decomposition}
	\mathbb{P}(w_1w_2...w_N) = \prod_{i = 1}^N \mathbb{P}(w_i | w_1w_2...w_{i - 1})
\end{gather}
From now on, I will use $w_i^j$ as shorthand for the sequence of words $w_iw_{i+1}...w_j$, where $i \leq j$. \\

The predictive power of language models is useful in a variety of applications. For example, in speech recognition, if the speech recogniser has estimated two candidate word sequences from an acoustic signal; \tit{`it's not easy to wreck a nice beach'} and \tit{`it's not easy to recognise speech'}, then a language model can be used to determine that the second candidate is more probable than the first. Language models are also used in machine translation, handwriting recognition, part-of-speech tagging and information retrieval.

\begin{alignat*}{2}
	\overbrace{\tit{do you want to grab a }}^{w_1^6}&\overbrace{\underline{\tit{drink}}}^{w_7} &&\hspace{0.5mm}\overbrace{(0.327)}^{\mathbb{P}(w_7 | w_1^6)} \\
	&\tit{coffee} &&\ (0.211) \\
	&\tit{bite} &&\ (0.190) \\
	&\tit{spot} &&\ (0.084) \\
	&\vdots &&\ \ \vdots
...
My project focuses on language modelling in the context of text prediction. For instance, if a user has typed \tit{`do you want to grab a '}, then a language model could be used to suggest probable next words such as \tit{`coffee'}, \tit{`drink'} or \tit{`bite'}. \\

In this project, I focused in depth on the two most prominent types of language model: $n$-gram models and recurrent neural network (RNN)-based models. I implemented and benchmarked $n$-gram models coupled with five different smoothing techniques and RNN models of three different flavours on a variety of datasets. I also assessed the trade-offs between prediction accuracy and resource consumption for my model implementations, which became relevant when I applied them in the context of a mobile keyboard. \\

One problem with existing language models is that their next-word predictions tend to be less accurate when they are presented with text containing errors. Unfortunately, humans are error-prone and inevitably do make mistakes when typing. In this dissertation, I present a novel extension to an RNN-based language model that improves its predictions on error-prone text.

\chapter{Preparation} \label{preparation}

My preparation consisted of background reading and thoroughly understanding various language models, as well as planning how to tie them together in an efficient implementation.
...
An $n$-gram is a sequence of $n$ words. $n$-gram language models provide a way of estimating the probability, $\mathbb{P}(w_i | w_1^{i - 1})$, of seeing a word, $w_i$, given the sequence of words that precede it, $w_1^{i - 1}$. They make the Markov assumption that $w_i$ only depends on the previous $(n - 1)$ words. That is:
\begin{gather}
	\mathbb{P}(w_i | w_1^{i - 1}) \approx \mathbb{P}(w_i | w_{i - n + 1}^{i - 1})
\end{gather}
...
Add-one smoothing~\cite{add1_smoothing:johnson1932} simply involves adding 1 to each of the $n$-gram counts, and adding $|V|$ to the denominator ensures the probabilities sum to 1:
\begin{gather}
...
Katz smoothing~\cite{katz_smoothing:katz1987} is a popular smoothing technique based on the Good-Turing estimate~\cite{good_turing:good1953}, which states that an $n$-gram that occurs $r$ times should be treated as occurring $r^*$ times, where:
\begin{gather}
...
\end{gather}
Katz smoothing is then defined as:
...
\end{gather}
In practice, the infinite sum in Equation~\ref{eq:gt} cannot be computed. A modified form of Equation~\ref{eq:katz} is presented in Katz's original paper, which makes an approximation to get around this issue.
...
It is important that the activation function, $\phi$, is chosen to be non-linear. Any combination of linear operators is linear, which means that any linear MLP with multiple hidden layers is equivalent to an MLP with a single hidden layer. Non-linear neural networks, on the other hand, are more powerful, and can gain considerable performance by adding successive hidden layers to re-represent the input data at higher levels of abstraction~\cite{dbn:hinton2006, scaling:bengio2007}. In fact, it has been shown that a non-linear MLP with a single hidden layer containing a sufficient number of neurons can approximate any continuous function on a compact input domain to arbitrary precision~\cite{universal_approximators:hornik1989}. \\

Frequently used activation functions include the sigmoid and the hyperbolic tangent functions. Their step-like shape is attributed to the fact that activation functions were originally designed to model the firing activity of neurons. Another important property of these functions is that they are differentiable, which allows for the network to be trained using \tit{gradient descent}, which is discussed below. \\
...
In order for an RNN to take a sequence of words as input, each word must first be mapped to a vector representation, known as a \tit{word embedding}, which represents a point in a high-dimensional space. Ideally, similar words will be assigned points which are closer together, so that it is easier for the RNN to generalise to words that it has not seen before. \\

In this dissertation, I get the RNN to learn good word embeddings through the application of backpropagation and gradient descent to the vector elements.
...
\begin{tabular}{L{1cm} L{14cm}}
	...
	\tbf{R3} & A simple console application is developed to demonstrate the capability of the aforementioned LMs in the context of next-word prediction. \\ \hline
\end{tabular}
\end{center}
...
\begin{tabular}{L{1cm} L{14cm}}
	\hline
	\tbf{E1} & Explore extensions to existing language models to improve their performance on error-prone text. \\ \hline
	\tbf{E2} & Build a mobile keyboard that exhibits the functionality of the language models. \\ \hline
\end{tabular}
\end{center}

All requirements and extensions depend on \tbf{R1}, so I implemented that first. Afterwards, I built \tbf{R2} and \tbf{R3} in parallel before completing \tbf{E1} and \tbf{E2}.
...
I hosted my project in a repository on GitHub\footnote{\href{https://github.com/DevanKuleindiren/language-modelling}{https://github.com/DevanKuleindiren/language-modelling}}, used Git for version control, and used Bazel for running builds and tests on my project.

\subsubsection{Machine Learning}

I chose to use TensorFlow~\cite{tensorflow:abadi2016}, an open source machine learning library, to assist in the implementation of my recurrent neural networks. There are two key reasons for this:
\begin{enumerate}
\item
...
In order to train large scale models on NVIDIA GPUs, I used the University's High Performance Computing service. This uses SLURM for cluster management and job scheduling.
...
This chapter details how I implemented a variety of $n$-gram and RNN-based language models, how I built the mobile keyboard on iOS and how I extended an existing language model to improve its performance on error-prone text.
...
public:
    ...
    // Initialise `preds' with the top k most probable next words that follow `seq', in
...
All new code was tested before being committed to the codebase, and new test cases were immediately added upon discovery of any bugs.
...
At the very start of the network is a word embedding lookup table, that maps words to vectors that represent points in a high dimensional space. I set the number of vector elements in each word embedding to be the same as the number of hidden neurons in each hidden layer of the RNN. This was a configurable parameter, $H$, which was typically set to 256. \\

Each word embedding retrieved from the lookup table is then fed into the input layer of the RNN, which is followed by the hidden layers and the output layer. Hermans and Schrauwen~\cite{deep_rnn:hermans2013} demonstrate that RNNs with more than one hidden layer naturally employ a temporal hierarchy, where lower layers capture short term interactions among words and higher layers reflect longer-term dependencies. For this reason, I chose to have two hidden layers in my architecture. \\
...
\centering
\begin{tikzpicture}[
	state/.style={circle, draw, inner sep=0pt, minimum width=15mm, thick},
	arrow/.style={->, >=LaTeX, font=\scriptsize},
]

\node[state] (state1) at (-3, 0) {$C = \epsilon$};
\node[state] (state2) at (3, 0) {$C \neq \epsilon$};
\node[left=40mm of state1] (start) {};

\draw[arrow, line width=0.4mm] (start) to node [above] {$\text{start}\ \big|\ \text{query LM}$} (state1);

\draw[arrow] (state1) [out=-110, in=-160, looseness=8] to node [below, -] {$\backdel, |S| \neq 1\ \Big|\ -$} (state1);
\draw[arrow] (state1) [out=160, in=110, looseness=8] to node [above, -] {$\textvisiblespace\ \Big|\ -$} (state1);

\draw[arrow, line width=0.4mm] (state1) [out=30, in=150] to node [above, -] {$c\ \Big|\ \text{filter predictions}$} (state2);
\draw[arrow] (state1) [out=60, in=120] to node [above, -] {$\backdel, |S| = 1\ \Big|\ \text{query LM; filter predictions}$} (state2);

\draw[arrow, line width=0.4mm] (state2) [out=70, in=20, looseness=8] to node [above, -] {$c\ \Big|\ \text{filter predictions}$} (state2);
\draw[arrow] (state2) [out=-20, in=-70, looseness=8] to node [below, -] {$\backdel, |C| \neq 1\ \Big|\ \text{filter predictions}$} (state2);

\draw[arrow, line width=0.4mm] (state2) [out=-150, in=-30] to node [below, -] {$\textvisiblespace\ \Big|\ \text{query LM}$} (state1);
\draw[arrow] (state2) [out=-120, in=-60] to node [below, -] {$\backdel, |C| = 1\ \Big|\ \text{filter predictions}$} (state1);

\end{tikzpicture}
\caption{An FSM representing the language model interaction. The thick arrows denote the path taken when typing without any deletions or unnecessary spaces.}
\label{fig:mobile_keyboard_fsm}
\end{figure}
$c$, \textvisiblespace\ and \keys{\backdel} represent a non-space character insertion, a space insertion and a backspace respectively. `Query LM' represents the process of querying the language model with $W$. The language model vocabulary is stored in a character-level trie, and the probabilities for each word from the query result are inserted at the nodes of the trie. `Filter predictions' represents the process of traversing this trie using $C$ and taking the top three most probable words on the current branch. This approach finds the most probable next-word predictions that begin with the characters that the user has already typed for the current word, whilst avoiding unnecessary language model queries. \\
...
An issue with existing language models is that their predictions suffer when presented with error-prone text. For example, consider the two word sequences `\tit{the weather is}' and `\tit{the whether is}'. Whilst in the first case a language model might successfully predict words such as `\tit{sunny}' or `\tit{nice}', in the second case it will likely struggle to suggest reasonable word candidates. Evidence to support this claim is given in Table~\ref{table:error_correcting_results} in Section~\ref{error_prone_evaluation}. \\

In the context of text prediction, it is unfortunately very rare that a human user will not make any mistakes, which makes this an important issue to address. In this section, I propose an extension to a language model which aims to improve its performance on text containing errors.

\subsection{The Approach}

My approach is to add a layer of error correction between the input text and the language model. There are lots of different types of error that can occur in the English language, but my implementation only focuses on correcting spelling mistakes. \\
...
\end{align*}
I implemented a benchmarking framework that can return a series of metrics which are outlined below. The first two are concerned with \tbf{Q1} and the latter three relate to \tbf{Q2}.
...
One issue with perplexity is that it is undefined if $\mathbb{P}_L(w_i | w_1^{i-1})$ is 0 at any point. This is problematic for $n$-gram models with no smoothing, since they assign 0 probabilities to unseen $n$-grams. In order to make this model comparable to the others under this metric, I replaced probability values of 0 with the small constant \ttt{1e-9}.
...
This is measured as the amount of physical memory in megabytes occupied by the process running the language model, retrieved from the \ttt{resident\_size} member of the \ttt{task\_basic\_info} struct\footnote{\href{http://web.mit.edu/darwin/src/modules/xnu/osfmk/man/task\_basic\_info.html}{http://web.mit.edu/darwin/src/modules/xnu/osfmk/man/task\_basic\_info.html}} for the current process.
...
The PTB~\cite{ptb:marcus1993} is a widely-adopted dataset of 1,036,580 words for measuring the quality of language models, derived from text from the Wall Street Journal. I used it for all tests in which the size of the training data is fixed.

\subsubsection{One Billion Word (1BW) Benchmark}

This is a much larger dataset produced by Google of approximately 1 billion words~\cite{1bw:chelba2013}. I used this dataset for tests in which the size of the training data is a variable under investigation.

\subsubsection{First Certificate in English (FCE) Dataset}

The FCE dataset is a subset of the Cambridge Learner Corpus, which is a dataset of exam scripts written by candidates sitting an FCE examination in 2000 and 2001~\cite{clc:yannakoudakis2011}. In this project I make use of a preprocessed version of this dataset, consisting of 88,130 words, in which error-prone and error-free versions of the exam scripts are aligned line by line in separate files. I used this dataset when exploring the performance of language models on error-prone text.
...
Cross-entropy is the binary logarithm of perplexity, and lower perplexity scores indicate better prediction accuracy. With this in mind, it is clear that Kneser-Ney-based smoothing offers the best prediction performance amongst the $n$-gram models. \\
...
I implemented three RNN-based language models which differ in their cell architecture: vanilla RNN, Gated Recurrent Unit and Long Short-Term Memory. \\

Some datasets are more predictable than others, which means that metrics like perplexity and average-keys-saved depend on which dataset is used. In order to produce results that are comparable with the work of other authors, I have tested my language models on the PTB dataset, as shown in Table~\ref{table:ptb}. The RNN-based models used 2 hidden layers each with 256 neurons. Average-keys-saved, an expensive metric to compute, was only taken over the first 1000 words of the test set. \\

\begin{table}[h]
\begin{adjustbox}{center}
\begin{tabular}{| L{5.2cm} | C{2.2cm} | C{2cm} | C{2cm} |  C{2cm} | C{2cm} |}
	\hline
	\tbf{Language Model} & \tbf{Perplexity} & \tbf{Average-Keys-Saved} & \tbf{Memory Usage (MB)} & \tbf{Training Time (min,secs)}$^*$ & \tbf{Average Inference Time (ms)} \\ \hline
...
\end{adjustbox}
\begin{center}
	{\footnotesize\tit{$^*$The $n$-gram models were trained on my laptop, whereas the neural models were trained on a GPU cluster on the High Performance Computing service.}}
\end{center}
\caption{A benchmark of various language models on the PTB dataset.}
\label{table:ptb}
\end{table}

There is a lot of information that can be drawn from Table~\ref{table:ptb}, so I will highlight only the most important and interesting points:
\begin{itemize}
\item
	The RNN-based language models outperform the $n$-gram models in terms of prediction accuracy. In my implementation, they run slightly faster at inference time and also consume less memory, although this is partly because the $n$-gram models are entirely unpruned. Obtaining the correct hyperparameters and training the RNN-based language models, however, does take significantly longer.
\item
	Mikolov benchmarked 3-gram + MKN, 5-gram + MKN, a 1-layer vanilla RNN and a 1-layer LSTM model with 100 hidden neurons on the PTB dataset, for which he obtained perplexity scores of 148.3, 141.2, 124.7 and 115 respectively~\cite{rnn_ptb:mikolov2012, lstm_ptb:mikolov2014}. All of these results closely match my own, with slightly more deviation in the RNN-based results due to his models having a different number of neurons and hidden layers.
\item
...
\item
	LSTM yields marginally better prediction accuracy than the GRU architecture, but requires more memory. Apart from the memory overhead, GRUs outperform the vanilla RNN architecture.
\item
...
As shown in Figure~\ref{fig:cross_entropy_training_set_size}, I also investigated the effect of the training set size on prediction accuracy. The RNN-based language models consistently outperform the $n$-gram models. This difference is most significant for small training sets, and starts to become more pronounced again once the training set grows large beyond $10^6$ words. \\

Finally, I present a qualitative comparison of a smoothed $n$-gram model with an RNN-based model in Figure~\ref{fig:generated_sentences}. \\
...
In this section I explore the trade-offs between language model resource consumption and prediction performance. \\
...
\end{subfigure}
\caption{The effect of changing the vocabulary size for models trained on the PTB dataset.}
\label{fig:average_keys_saved_vs_minimum_frequency}
\end{figure}

I altered the vocabulary size by changing a parameter called the \tit{minimum frequency}, which defines the minimum number of times a word must appear in the training set to be considered part of the language model vocabulary. \\
...
\end{semilogxaxis}
\end{tikzpicture}
\caption{The effect of changing the number of hidden neurons in a 2-layer vanilla RNN trained on the PTB dataset.}
\label{fig:average_keys_saved_vs_number_of_hidden_neurons}
\end{figure}

Comparing figures~\ref{fig:average_keys_saved_vs_minimum_frequency} and~\ref{fig:average_keys_saved_vs_number_of_hidden_neurons}, it can be seen that much greater reductions in memory usage can be achieved for a given loss in average-keys-saved by decreasing the number of hidden neurons. Dropping from 256 to 32 hidden neurons in each layer gives a memory saving of 132.31 MB, with an average-keys-saved loss of 0.04758. It is intriguing that such a reduced vanilla RNN model can achieve a better average-keys-saved score than the 5-gram model with Katz smoothing from Table~\ref{table:ptb}. \\

Of course, there are several other techniques that can be used to lower the memory and CPU pressure of language models on mobile platforms. These include, but are not limited to: reducing the number of hidden layers; using 16-bit floats for model parameters; removing the softmax layer from the RNN; and memory-mapping the model.

\subsection{On Error-Prone Text} \label{error_prone_evaluation}

In this section I present the performance of my language model extension on error-prone text. The goal is to produce a language model whose predictions are not deterred by errors in the input text, as illustrated in Figure~\ref{fig:error_prone_to_error_free_rnn}. \\
...
I used the FCE dataset, which consisted of six files:
\begin{center}
\begin{tabular}{c c}
	\ttt{train.correct} & \ttt{train.incorrect} \\
	\ttt{valid.correct} & \ttt{valid.incorrect} \\
	\ttt{test.correct} & \ttt{test.incorrect}
\end{tabular}
\end{center}
The training, validation and test sets all consisted of one file containing uncorrected text and another file containing the corresponding corrected text. I trained a 2-layer LSTM-based language model with 256 hidden neurons on \ttt{train.correct}, and used \ttt{valid.correct} to guide the learning rate decay. In order to focus the evaluation on the error-correcting ability of my language models, I removed any pairs of lines from the test set files that were identical. I also removed any pairs of lines that contained corrections that involve the insertion or deletion of words, because my model was not designed to handle these types of errors, as explained in Section~\ref{error_correcting_lm}. This left 6672 words in each of the test files. \\

The ideal error-correcting language model would predict words as if the input were not error-prone, therefore, to obtain an \tit{upper bound} in performance, I evaluated the LSTM-based language model using \ttt{test.correct} for the input words and \ttt{test.correct} for the target words. In order to establish a \tit{lower bound}, I evaluated the performance of the same model using \ttt{test.incorrect} for the input words and \ttt{test.correct} for the target words. This model did not contain any error-correcting capabilities. \\
...
\end{tabular}
\end{adjustbox}
\caption{The performance of my error-correcting language model with respect to the upper and lower bound, using the FCE dataset.}
\label{table:error_correcting_results}
\end{table}

The performance of my error-correcting language model that was outlined in Section~\ref{error_correcting_lm} is given in Table~\ref{table:error_correcting_results}. It can be seen that it improves predictions on text containing errors that do not require the insertion or deletion of words. The best result arises from an edit distance threshold of 1, which seems attributed to the fact that a larger threshold allows for more false corrections, as shown in Figure~\ref{fig:error_correcting_false}.
...
My implementation of existing language models saw a level of performance that was comparable with measures made by existing authors. For example, my modified Kneser-Ney smoothing $n$-gram model, vanilla RNN model and LSTM model all matched the results from Mikolov on the Penn Tree Bank dataset~\cite{rnn_ptb:mikolov2012, lstm_ptb:mikolov2014}. This allowed me to make comprehensive and reliable comparisons between each of my models, and assess the trade-offs between accuracy and resource consumption. My work also saw such models complement one another, in that the interpolation of their probability distributions surpassed the performance of all of my stand-alone models, achieving state-of-the-art prediction accuracy. \\

I explicitly demonstrated that language model performance is compromised by error-prone text. Whilst I made some modifications to an existing model that saw an increase in performance on such text, there is still room for improvement. If I were given more time to work on the project, I would investigate ways to cater for non-spelling corrections. From the CoNLL shared tasks of 2013~\cite{error_correction2013:ng2013} and 2014~\cite{error_correction2014:ng2014}, it is clear that there is a plethora of methods that already exist for grammatical error-correction. It would be interesting to see which of these could be applied to improve the performance of language models on error-prone text. \\
...
\begin{gather*}
	\delta_i^l = \sum_j \delta_j^{l+1} \frac{\partial z_j^{l+1}}{\partial z_i^l} = \sum_j \delta_j^{l+1} \frac{\partial z_j^{l+1}}{\partial a_i^l} \frac{\partial a_i^l}{\partial z_i^l} = \phi'(z_i^l) \sum_j \delta_j^{l+1} w_{i,j}^l
\end{gather*}
as required. \\

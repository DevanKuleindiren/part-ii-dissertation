...
In my implementation, I precompute and store all of these parameters in a temporary word-level trie, as shown in figure \todo{X}. This way, each parameter is only ever computed once and is accessible in $O(n)$ time. Given that I only consider $n$-grams for $n=2$, 3, 4 or 5, the access time practically constant.
...
\begin{tikzpicture}[
	...
	\draw[-, thick] (c2) -- ([xshift=0.35mm, yshift=0.4mm]inside.south west);
\end{tikzpicture}
\caption{An example instance of the \ttt{CountTrie} class.}
...
Below, I explain how I implemented my RNN-based language models. To do this, I first give a brief overview of TensorFlow, then I explain the structure of the RNN I use and how it applies to language modelling, and finally I give the details behind the cell architectures I used, word embeddings and parameter tuning.
...
Talk about computing gradients and applying backprop. \\

Talk about exporting graph to proto and running in C++. \\
...
Explain the structure of the network, such as:
\begin{itemize}
\item
	The way input words are converted to word vectors.
\item
	The number of layers and their size.
\item
	The fact that there are 2 layers of RNN cells followed by a fully connected layer which is finally fed through softmax.
\item
	Explain the output neurons, what they represent and the need (in some cases) for softmax.
\end{itemize}

\subsection{Long Short-Term Memory and Gated Recurrent Units}
\begin{itemize}
\item
...
As a project extension, I decided to apply one of my language models in a practical context by building a mobile keyboard. Below I describe how I went about doing this, as well as the lessons I learnt in the process. \\
...
\todo{Talk about tricks to make smaller.} \\

\subsection{Updating Language Model Predictions On the Fly}

A mobile keyboard should present the user with next-word predictions that are updated as they type. Querying the language model is a relatively expensive operation, so in my implementation I avoid unnecessary queries by only asking for next-word probabilities whenever the user presses the spacebar, changes their cursor position to another word or  It is only necessary to query the language model when the user is about to start typing a new word, namely, when they press the space bar\footnote{There are other edge cases where it is also required, such as when they change the position of their cursor or delete a series of words. These are all handled in my implementation.}. 
